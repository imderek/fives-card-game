...
const WinnerDeclaration = ({ 
    ...
    const isCurrentUserWinner = game.winner_id === currentUser.id;
    const winnerRevealed = countUpComplete;
    ...
    }, [isCurrentUserWinner, triggerConfetti]);
    ...
    return (
        <div className="w-full flex flex-col animate-enter-scale">
            <div className="winner-scores my-1 mx-4 md:mx-0 px-3 pt-3 pb-3 bg-white rounded-lg relative z-40">
                <h1 className={`mt-1 ${winnerRevealed ? 'mb-4' : 'mb-1'} leading-7 text-slate-900 text-center flex flex-col items-center justify-center gap-2
                text-xl leading-none font-bold
                `}>
                    {winnerRevealed && isCurrentUserWinner && (
                        <div className="animate-scale-up-win w-9 h-9 bg-amber-500 text-white rounded-full flex items-center justify-center">
                            <i className="fa fa-trophy text-xl relative top-[.075rem]"></i>
                        </div>
                    )}
                    {winnerRevealed ? 
                        (isCurrentUserWinner ? "You Won!" : "You Lost") : 
                        <div className="mb-2 relative top-0.5 font-bold" role="status">
                            <svg aria-hidden="true" className="inline mr-2 w-9 h-9 animate-spin text-slate-300 fill-amber-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className="block mt-4">Calculating scores</span>
                        </div>
                    }
                </h1>
