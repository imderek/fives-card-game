...
const PlayerHand = ({ cards, isCurrentPlayer, canPlay, onPlayCard, onDiscard, canDiscard, facedown = false, handScore = 0, winner, discardPile = [], opponentName = 'Opponent' }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 relative">
      <div className={`${winner ? 'mb-4' : 'mb-0'} ${(!isCurrentPlayer && !winner) ? 'opacity-90' : ''} relative flex items-end justify-center ${facedown ? '-space-x-4' : 'top-[1.8rem] space-x-[-1.5rem]'}`}>
      ...
      {handScore > 0 && (
        <div className="absolute -top-[.4rem] inset-0 flex items-center justify-center z-30">
          <div className="p-2 py-1 bg-slate-500 rounded-md text-xs sm:text-sm font-medium shadow-lg text-white animate-enter-scale">
            +{handScore}
          </div>
        </div>
      )}
